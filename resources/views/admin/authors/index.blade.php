...
@section('title')
<h1 class="text-3xl text-black pb-6">{{__('Authors')}}</h1>
@endsection

@section('content')
    <a href="{{route('authors.create')}}" class="text-white bg-slate-700 hover:bg-slate-800 focus:ring-4 focus:outline-none focus:ring-slate-300 font-medium rounded-lg text-md px-4 py-2">+ {{__('Add New Authors')}}</a>
    <div class="flex justify-between my-2 items-center">
        <form class=" w-26 mx-auto" class="max-w-md mx-auto" action="{{route('authors.index')}}" method="GET">
        ...
        <form class=" mx-auto min-w-60 max-w-80" action="{{route('authors.searchAuthors')}}" method="GET">
            <x-search/>
        </form>
    </div>
            ...
            <tr class="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50">
                ...
                <td class="px-6 py-4">
                    <div class="flex justify-center items-center flex-wrap gap-1">
                        <a href="{{route('authors.edit',$author)}}"  class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-2 py-1">{{__("Edit")}}</a>
